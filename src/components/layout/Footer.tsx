import { Link } from "react-router-dom";

export default function () {
  return (
    <footer className="w-screen h-auto bottom-0 bg-white">
      <div className="w-full">
        <div className="w-full flex flex-row gap-[78px] pb-[67px] pt-[67px] --global-padding">
          <div className="flex-[1.4]">
            <h1 className="text-[#00329B] font-bold text-[16px]">LOGO</h1>

            <p className="text-[#515151] mt-7 text-sm">
              Lorem Ipsum is simply dummy text of the printing and typesetting
              industry.
            </p>

            <p className="text-[#515151] mt-7 text-sm">@Lorem</p>
          </div>

          <div className="flex-[0.4] text-sm">
            <p className="text-[17px] font-medium text-black">About Us</p>

            <ul className="flex flex-col gap-2 mt-5">
              <li>
                <Link to="/" className="text-[#515151]">
                  Lorem
                </Link>
              </li>
              <li>
                <Link to="/" className="text-[#515151]">
                  Portfolio
                </Link>
              </li>
              <li>
                <Link to="/" className="text-[#515151]">
                  Careers
                </Link>
              </li>
              <li>
                <Link to="/" className="text-[#515151]">
                  Contact us
                </Link>
              </li>
            </ul>
          </div>

          <div className="flex-[1.2] text-sm">
            <p className="text-[17px] font-medium text-black">Contact us</p>

            <div className="mt-5">
              <span className="text-[#515151]">
                Lorem Ipsum is simply dummy text of the printing and typesetting
                industry.
              </span>
            </div>

            <div className="mt-5">
              <span className="text-[#515151]">+908 89097 890</span>
            </div>
          </div>

          <div className="flex-1 flex gap-4 h-fit">
            <Link to="/" className="h-fit">
              <span className="shadow flex w-[34px] h-[34px] items-center justify-center rounded-4xl bottom-0">
                <svg
                  width={12}
                  height={12}
                  viewBox="0 0 12 12"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    d="M7.9985 1.9925H9.094V0.0845C8.905 0.0585 8.255 0 7.498 0C5.91851 0 4.83651 0.9935 4.83651 2.8195V4.5H3.09351V6.633H4.83651V12H6.97351V6.6335H8.646L8.9115 4.5005H6.97301V3.031C6.97351 2.4145 7.1395 1.9925 7.9985 1.9925Z"
                    fill="black"
                  />
                </svg>
              </span>
            </Link>

            <Link to="/" className="h-fit">
              <span className="shadow flex w-[34px] h-[34px] items-center justify-center rounded-4xl bottom-0">
                <svg
                  width={13}
                  height={13}
                  viewBox="0 0 13 13"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <g clipPath="url(#clip0_2_1232)">
                    <path
                      d="M9.481 0H3.51898C1.57858 0 0 1.57858 0 3.51898V9.4811C0 11.4214 1.57858 13 3.51898 13H9.4811C11.4214 13 13 11.4214 13 9.4811V3.51898C13 1.57858 11.4214 0 9.481 0V0ZM12.2379 9.4811C12.2379 11.0012 11.0012 12.2379 9.481 12.2379H3.51898C1.99881 12.2379 0.762114 11.0012 0.762114 9.4811V3.51898C0.762114 1.99881 1.99881 0.762114 3.51898 0.762114H9.4811C11.0012 0.762114 12.2379 1.99881 12.2379 3.51898V9.4811Z"
                      fill="black"
                    />
                    <path
                      d="M6.49989 2.94531C4.53986 2.94531 2.94531 4.53986 2.94531 6.49989C2.94531 8.45993 4.53986 10.0545 6.49989 10.0545C8.45993 10.0545 10.0545 8.45993 10.0545 6.49989C10.0545 4.53986 8.45993 2.94531 6.49989 2.94531ZM6.49989 9.29236C4.96019 9.29236 3.70743 8.03969 3.70743 6.49989C3.70743 4.96019 4.96019 3.70743 6.49989 3.70743C8.03969 3.70743 9.29236 4.96019 9.29236 6.49989C9.29236 8.03969 8.03969 9.29236 6.49989 9.29236Z"
                      fill="black"
                    />
                    <path
                      d="M10.1394 1.68311C9.56023 1.68311 9.08911 2.15432 9.08911 2.73344C9.08911 3.31266 9.56023 3.78388 10.1394 3.78388C10.7187 3.78388 11.1899 3.31266 11.1899 2.73344C11.1899 2.15422 10.7187 1.68311 10.1394 1.68311ZM10.1394 3.02166C9.98056 3.02166 9.85122 2.89233 9.85122 2.73344C9.85122 2.57445 9.98056 2.44522 10.1394 2.44522C10.2984 2.44522 10.4278 2.57445 10.4278 2.73344C10.4278 2.89233 10.2984 3.02166 10.1394 3.02166Z"
                      fill="black"
                    />
                  </g>
                  <defs>
                    <clipPath id="clip0_2_1232">
                      <rect width={13} height={13} fill="white" />
                    </clipPath>
                  </defs>
                </svg>
              </span>
            </Link>

            <Link to="/" className="h-fit">
              <span className="shadow flex w-[34px] h-[34px] items-center justify-center rounded-4xl bottom-0">
                <svg
                  width={13}
                  height={13}
                  viewBox="0 0 13 13"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    d="M13 2.46919C12.5166 2.68125 12.0014 2.82181 11.4644 2.89006C12.0169 2.56019 12.4386 2.04181 12.6368 1.417C12.1217 1.72412 11.5529 1.94106 10.9468 2.06213C10.4577 1.54131 9.76056 1.21875 9.00006 1.21875C7.52456 1.21875 6.33669 2.41638 6.33669 3.88456C6.33669 4.09581 6.35456 4.29894 6.39844 4.49231C4.18275 4.38425 2.22219 3.32231 0.905125 1.70463C0.675187 2.10356 0.540313 2.56019 0.540313 3.05175C0.540313 3.97475 1.01562 4.79294 1.72413 5.26662C1.29594 5.2585 0.875875 5.13419 0.52 4.93837C0.52 4.9465 0.52 4.95706 0.52 4.96762C0.52 6.26275 1.44381 7.3385 2.65525 7.58631C2.43831 7.64563 2.20187 7.67406 1.9565 7.67406C1.78587 7.67406 1.61362 7.66431 1.45194 7.62856C1.79725 8.684 2.77713 9.45994 3.94225 9.48512C3.0355 10.1944 1.88419 10.6218 0.637812 10.6218C0.41925 10.6218 0.209625 10.6121 0 10.5852C1.18056 11.3466 2.57969 11.7812 4.0885 11.7812C8.99275 11.7812 11.674 7.71875 11.674 4.19737C11.674 4.07956 11.6699 3.96581 11.6643 3.85287C12.1932 3.4775 12.6376 3.00869 13 2.46919Z"
                    fill="black"
                  />
                </svg>
              </span>
            </Link>

            <Link to="/" className="h-fit">
              <span className="shadow flex w-[34px] h-[34px] items-center justify-center rounded-4xl bottom-0">
                <svg
                  width={11}
                  height={11}
                  viewBox="0 0 11 11"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <g clipPath="url(#clip0_2_1246)">
                    <path
                      d="M10.9973 11V10.9996H11.0001V6.96531C11.0001 4.99173 10.5752 3.47144 8.26795 3.47144C7.15879 3.47144 6.41445 4.0801 6.11058 4.65714H6.07849V3.65569H3.89087V10.9996H6.16879V7.36314C6.16879 6.40569 6.35029 5.47985 7.536 5.47985C8.70429 5.47985 8.7217 6.57252 8.7217 7.42456V11H10.9973Z"
                      fill="black"
                    />
                    <path
                      d="M0.181396 3.65601H2.46206V10.9999H0.181396V3.65601Z"
                      fill="black"
                    />
                    <path
                      d="M1.32092 0C0.591708 0 0 0.591708 0 1.32092C0 2.05013 0.591708 2.65421 1.32092 2.65421C2.05013 2.65421 2.64183 2.05013 2.64183 1.32092C2.64138 0.591708 2.04967 0 1.32092 0V0Z"
                      fill="black"
                    />
                  </g>
                  <defs>
                    <clipPath id="clip0_2_1246">
                      <rect width={11} height={11} fill="white" />
                    </clipPath>
                  </defs>
                </svg>
              </span>
            </Link>
          </div>
        </div>

        <div
          className="pt-7 pb-7 text-center"
          style={{ borderTop: "1px solid #D2D2D2" }}
        >
          <span className="text-[#686868] text-sm">
            Copyright ® 2021 Lorem All rights Reserved
          </span>
        </div>
      </div>
    </footer>
  );
}
